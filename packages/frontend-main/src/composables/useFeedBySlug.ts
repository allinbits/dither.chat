...
interface UseFeedBySlugOptions {
  slug: Ref<string>;
}

export function feedBySlug(queryClient: QueryClient, { slug }: UseFeedBySlugOptions) {
  ...
  return infiniteQueryOptions({
    queryKey: ['feed', slug, debouncedFilterAmount],
    queryFn: async ({ pageParam = 0 }) => {
      const res = await fetch(
        `${apiRoot}/feed/${slug.value}?offset=${pageParam}&limit=${LIMIT}&minQuantity=${debouncedFilterAmount.value}`,
      );
...
export function useFeedBySlug(options: UseFeedBySlugOptions) {
  return useInfiniteQuery(feedBySlug(useQueryClient(), options));
}
