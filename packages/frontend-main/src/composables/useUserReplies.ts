...
export const userReplies = (params: Params) => {
    ...
    return infiniteQueryOptions({
        ...
        queryFn: async ({ pageParam = 0 }) => {
            ...
            const res = await fetch(`${apiRoot}/user-replies?address=${params.userAddress.value}&offset=${pageParam}&limit=${LIMIT}&minQuantity=${Math.trunc(debouncedFilterAmount.value)}`);
