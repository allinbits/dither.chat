    ...
    <template v-if="connectedState">
      <Popover>
        <PopoverTrigger class="w-full">
          <UserAvatarUsername :user-address="address" size="sm" disabled class="h-[52px] px-4 gap-3 rounded-sm hover:bg-accent active:bg-accent transition-colors">
            <template #username="{ display }">
              <span class="text-xs font-normal truncate" :title="display">{{ display }}</span>
            </template>
          </UserAvatarUsername>
        </PopoverTrigger>
