...
<template>
  <Avatar :class="cn(size === 'lg' ? 'size-[64px]' : size === 'sm' ? 'size-[32px]' : 'size-[40px]', !disabled && 'hover:opacity-60 transition-opacity', 'z-10')">
    <AvatarImage v-if="userAddress" :class="cn('!transition-[filter]', mode === 'dark' && 'filter-[invert(0.96))]')" :src="`https://api.dicebear.com/9.x/notionists-neutral/svg?seed=${userAddress}`" alt="user-avatar-image" />
