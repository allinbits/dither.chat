...
async function handleSubmit() {
    if (!popovers.state.dislike) {
        return;
    }

    const result = await wallet.dither.dislike(popovers.state.dislike.hash, BigInt(photonValue.value).toString());
    if (!result.broadcast) {
...
<template>
  <div>
    <Dialog :open="popovers.state.dislike != null" @update:open="handleClose" v-if="popovers.state.dislike">
      <DialogContent>
        <DialogTitle>{{ $t('components.PopupTitles.dislikePost') }}</DialogTitle>
        <DialogDescription>{{ shorten(popovers.state.dislike.hash) }}</DialogDescription>
