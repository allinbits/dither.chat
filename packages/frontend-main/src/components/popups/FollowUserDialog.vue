...
async function handleSubmit() {
    if (!popups.state.follow) {
        return;
    }

    const result = await wallet.dither.follow(popups.state.follow, BigInt(photonValue.value).toString());
    if (!result.broadcast) {
...
<template>
  <Dialog :open="popups.state.follow != null" @update:open="handleClose" v-if="popups.state.follow">
    <DialogContent>
