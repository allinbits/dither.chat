...
const isShown = computed(() => !!popups.state.tipUser);

const { tipUser, txError, txSuccess } = useTipUser();
useTxNotification(isShown, 'Tip', txSuccess, txError);
...
<template>
  <Dialog
    v-if="popups.state.tipUser !== null && !isBroadcasting"
    :open="popups.state.tipUser != null && !isBroadcasting"
