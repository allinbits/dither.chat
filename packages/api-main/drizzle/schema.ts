...
export const FeedTable = pgTable(
    'feed',
    {
        ...
        quantity: bigint({ mode: 'bigint' }).default(sql`0`), // The total amount of tokens the user spent to post this message
        ...
        likes_burnt: bigint({ mode: 'bigint' }).default(sql`0`), // The amount of tokens burnt from each user who liked this post / reply
        dislikes_burnt: bigint({ mode: 'bigint' }).default(sql`0`), // The amount of tokens burnt from each user who disliked this post / reply
        flags_burnt: bigint({ mode: 'bigint' }).default(sql`0`), // The amount of tokens burnt from each user who wante dto flag this post / reply
...
export const DislikesTable = pgTable(
    'dislikes',
    {
        ...
        quantity: bigint({ mode: 'bigint' }).default(sql`0`),
