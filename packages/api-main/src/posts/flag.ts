...
export async function Flag(body: typeof FlagBody.static) {
    ...
    const [like_hash] = extractMemoContent(body.memo, 'dither.Flag');
    if (!like_hash) {
        return { status: 400, error: 'memo must contain a like address for dither.Like' };
    }

    try {
        const results = await db.select().from(FlagsTable).where(eq(FlagsTable.hash, like_hash));
        if (results.length >= 1) {
            ...
            await db.update(FlagsTable).set({
                data: sql`${FlagsTable.data} || ${author}::jsonb`,
            });
        } else {
            await db
                .insert(FlagsTable)
